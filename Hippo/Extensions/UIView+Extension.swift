...
extension TagListView {
    func setTagListViewProperty() {
        ...
        alignment = BumbleConfig.shared.appUserType == .agent ? .left : .right
        borderColo = BumbleConfig.shared.appUserType == .customer ? BumbleConfig.shared.theme.botTextAndBorderColor : UIColor.lightGray
        paddingY = 10
        paddingX = 20
        hippoBorderWidth = 0
        borderWidt = 1
        buttonBorderWidth = 1
        textColor = BumbleConfig.shared.appUserType == .customer ? BumbleConfig.shared.theme.botTextAndBorderColor : UIColor.lightGray
        selectedTextColor = BumbleConfig.shared.appUserType == .customer ? BumbleConfig.shared.theme.botTextAndBorderColor : UIColor.lightGray
        textFont = BumbleConfig.shared.theme.incomingMsgFont
    }
}
extension UINavigationController {
    func setTheme() {
        navigationBar.tintColor = BumbleConfig.shared.theme.headerTextColor
        navigationBar.barTintColor = BumbleConfig.shared.theme.headerBackgroundColor
        navigationBar.backgroundColor = BumbleConfig.shared.theme.headerBackgroundColor
        navigationBar.isTranslucent = false
        
        #if swift(>=4.0)
        var attributes: [NSAttributedString.Key : Any]  = [NSAttributedString.Key.foregroundColor: BumbleConfig.shared.theme.headerTextColor]
        attributes[NSAttributedString.Key.font] = BumbleConfig.shared.theme.headerTextFont
        #else
...
extension UIViewController {
    ...
    func showAlert(_ inController: UIViewController? = getLastVisibleController(), buttonTitle: String = BumbleStrings.ok, title: String?, message: String, preferredStyle: UIAlertController.Style = .alert, actionComplete: ((_ action: UIAlertAction) -> Void)?) {
