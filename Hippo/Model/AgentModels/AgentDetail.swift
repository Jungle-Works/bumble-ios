...
public enum HippoError: LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .threwError(message: let errorMessage):
            return errorMessage
        case .invalidEmail:
            return BumbleConfig.shared.strings.invalidEmail
        case .invalidInputData:
            return BumbleConfig.shared.strings.inputDataIsInvalid
        case .notAllowedForAgent:
            return BumbleConfig.shared.strings.notAllowedForAgent
        case .callClientNotFound:
...
extension AgentDetail {
    class func loginViaAuth(completion: @escaping (_ result: ResponseResult) -> Void) {
        guard BumbleConfig.shared.appUserType == .agent, let agentDetail = BumbleConfig.shared.agentDetail else {
        ...
        HTTPClient.makeConcurrentConnectionWith(method: .POST, para: params, extendedUrl: endpoint) { (responseObject, error, tag, statusCode) in
            ...
            let authToken = BumbleConfig.shared.agentDetail?.oAuthToken
            let app_type = BumbleConfig.shared.agentDetail?.app_type ?? AgentDetail.defaultAppType
            let attributes = BumbleConfig.shared.agentDetail?.customAttributes
            let isForking = BumbleConfig.shared.agentDetail?.isForking ?? false
            ...
            BumbleConfig.shared.agentDetail = detail
    ...
    class func loginAgentViaToken(completion: @escaping (_ result: ResponseResult) -> Void) {
        ...
        HTTPClient.makeConcurrentConnectionWith(method: .POST, para: params, extendedUrl: AgentEndPoints.loginViaToken.rawValue) { (responseObject, error, tag, statusCode) in
            ...
            if let businessProperty = data["business_property"] as? [String: Any] {
                ...
                BumbleConfig.shared.jitsiUrl = businessProperty["jitsi_url"] as? String
            }
...
extension AgentDetail {
    ...
    internal static func getParamsForLogout() -> [String: Any] {
        guard let agentDetail = BumbleConfig.shared.agentDetail else {
            return [:]
        }
...
//        params["device_details"] = getDeviceDetails()
        ...
        if let attributes = BumbleConfig.shared.agentDetail?.customAttributes {
...
extension AgentDetail {
    class var id: Int {
        get {
            guard let _agentID = BumbleConfig.shared.agentDetail?.id else { return -1 }
            return Int(_agentID)
        }
    }
}
