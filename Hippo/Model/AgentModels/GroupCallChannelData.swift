...
extension GroupCallChannel: SignalingClient {
    func sendSessionStatus(status: String, transactionId: String) {
        if status == "MISSED_GROUP_CALL"{
            BumbleConfig.shared.HippoSessionStatus?(.missed)
        }else if status == "JOIN_GROUP_CALL"{
            BumbleConfig.shared.HippoSessionStatus?(.sessionStarted)
        }else if status == "REJECT_GROUP_CALL"{
            BumbleConfig.shared.HippoSessionStatus?(.rejected)
        }else if status == "END_GROUP_CALL"{
            BumbleConfig.shared.HippoSessionStatus?(.sessionEnded)
        }else if status == "START_GROUP_CALL"{
           BumbleConfig.shared.HippoSessionStatus?(.agentSessionStarted)
        }
    }
