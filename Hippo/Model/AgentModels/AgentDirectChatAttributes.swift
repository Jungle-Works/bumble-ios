...
class AgentDirectChatAttributes: NSObject {
    ...
    private(set) var channelName = BumbleStrings.support
    ...
    init?(otherUserEmail: String) {
        ...
        guard email.isValidEmail() else {
            BumbleConfig.shared.log.debug(HippoError.invalidEmail.localizedDescription, level: .error)
            return nil
        }
    ...
    func getParamsToStartNewChat() -> [String: Any]? {
        guard let detail = BumbleConfig.shared.agentDetail else {
            return nil
        }
