...
class HippoActionMessage: HippoMessage {
    ...
    func setHeight() {
        ...
        if let attributtedMessage = responseMessage?.attributtedMessage {
            let nameHeight: CGFloat = BumbleConfig.shared.appUserType == .agent ? attributtedMessage.nameHeight : 0
...
//            HippoConfig.shared.log.debug(message, level: .custom)
    ...
    private func findButtonWidth(_ text: String) -> CGFloat {
        ...
        attributedText.addAttribute(.font, value: BumbleConfig.shared.theme.incomingMsgFont, range: range)
