...
//        }
    ...
    private func getSenderNameInThemeFont(appendWithString: String = "") -> NSMutableAttributedString {
        ...
        if isSelfMessage {
            let font = BumbleConfig.shared.theme.senderNameFont
            
            nameString.addAttribute(NSAttributedString.Key.foregroundColor, value: BumbleConfig.shared.theme.senderNameColor, range: NSMakeRange(0, nameString.string.length()))
    ...
    private func getAttributedStringWithThemeFont(aString: NSMutableAttributedString) -> NSMutableAttributedString {
        ...
        if isSelfMessage {
            let font = BumbleConfig.shared.theme.inOutChatTextFont
            ...
            let font = BumbleConfig.shared.theme.incomingMsgFont
            aString.addAttribute(NSAttributedString.Key.font, value: font, range: range)
            aString.addAttribute(NSAttributedString.Key.foregroundColor, value: BumbleConfig.shared.theme.incomingMsgColor, range: range)
            aString.addAttribute(NSAttributedString.Key.paragraphStyle, value: style, range: range)
            return aString
        }
    }
