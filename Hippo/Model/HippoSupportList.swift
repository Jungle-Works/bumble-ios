...
class HippoSupportList: NSObject {
    ...
    class func getDataFromJson(dict: [String: Any]) -> [HippoSupportList] {
        ...
        if !BumbleConfig.shared.ticketDetails.FAQName.isEmpty, let temp = getListwhere(key: BumbleConfig.shared.ticketDetails.FAQName, from: support_data), let lists = temp["list"] as? [[String: Any]] {
            list = lists
            data = temp
