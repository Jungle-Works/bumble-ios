...
class AllString{
    ...
    class func getAllStrings(completion: @escaping HippoResponseRecieved) {
        ...
        params["request_source"] = BumbleConfig.shared.appUserType == .agent ? 1 : 0
        params["app_secret_key"] = BumbleConfig.shared.appUserType == .agent ? BumbleConfig.shared.agentDetail?.appSecrectKey : BumbleConfig.shared.appSecretKey
    ...
    class func customerParsing(_ response: [String : Any]){
        ...
        if let newConversation = response["fugu_new_conversation"] as? String{
            BumbleConfig.shared.strings.newConversation = newConversation
        }
