...
public enum FuguEnvironment: Int {
   ...
   public static var selected: FuguEnvironment {
      if BumbleConfig.shared.baseUrl != SERVERS.liveUrl {
        return BumbleConfig.shared.baseUrl  == SERVERS.betaUrl ? .beta : .dev
      } else {
         return .live
      }
   }
...
enum IntegrationSource: Int {
    ...
    func getIcon() -> UIImage? {
        switch self {
        case .facebook:
            return BumbleConfig.shared.theme.facebookSourceIcon
        case .email:
            return BumbleConfig.shared.theme.emailSourceIcon
        case .sms:
            return BumbleConfig.shared.theme.smsSourceIcon
        case .whatsApp:
            return BumbleConfig.shared.theme.whatsappIcon
        case .normal:
            return nil
        }
    }
...
//        return [.normal, .imageFile, .privateNote, .assignAgent, .botText, .call, .attachment, .consent, .actionableMessage, .hippoPay]
    ...
    func isMessageTypeHandled() -> Bool {
        let typeHandled = BumbleConfig.shared.appUserType == .agent ? agentHandledMessages : customerHandledMessages
...
//        guard HippoConfig.shared.appUserType == .customer else {
        guard BumbleConfig.shared.appUserType == .customer || BumbleConfig.shared.appUserType == .agent else {
            return false
        }
...
enum UserType: Int {
    ...
    var isMyUserType: Bool {
        ...
        case .none, .system, .agent:
            return BumbleConfig.shared.appUserType == .agent
        default:
            return BumbleConfig.shared.appUserType == .customer
        }
    }
}
...
enum FuguEndPoints: String {
    case API_PUT_USER_DETAILS = "api/users/userLogin"
...
//    case getUnreadCount = "api/conversation/getUnreadCount"
    ...
    case getAnnouncements = "/api/campaign/getCampaigns"
    case createOneToOneChat = "api/chat/createOneToOneChat"
    
    case clearAnnouncements = "/api/campaign/clearCampaign"
