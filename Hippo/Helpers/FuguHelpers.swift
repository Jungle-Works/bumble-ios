...
//    }
//    ) {  (messageDict) in
//         BumbleConfig.shared.log.trace("UserChannel:: --->\(messageDict)", level: .socket)
...
//            if notificationType == NotificationType.message.rawValue && messageDict["channel_id"] as? Int != BumbleConfig.shared.getCurrentChannelId(){
//                if let channelId = messageDict["channel_id"] as? Int, let otherUserUniqueKey = ((messageDict["user_unique_keys"] as? [String])?.filter{$0 != BumbleConfig.shared.userDetail?.userUniqueKey}.first){
...
func subscribeMarkConversation(){
    ...
    if currentUserType() == .customer{
        markConversationChannel = BumbleConfig.shared.appSecretKey + "/" + "markConversation"
    }else{
        markConversationChannel = (BumbleConfig.shared.agentDetail?.appSecrectKey ?? "") + "/" + "markConversation"
    }
    ...
    if var channelList = FuguDefaults.object(forKey: DefaultName.agentTotalUnreadHashMap.rawValue) as? [String : Any], let totalUnreadCount = UserDefaults.standard.value(forKey: DefaultName.agentUnreadCount.rawValue) as? Int{
       //find if the channel exists in list
        if let channelUnreadCount = channelList["\(channelId)"] as? Int{
            ...
            BumbleConfig.shared.sendAgentUnreadCount(newUnreadCount)
            channelList.removeValue(forKey: "\(channelId)")
            BumbleConfig.shared.sendAgentChannelsUnreadCount(channelList.count)
       ...
       //find if the channel exists in list
        if let channelUnreadCount = channelList["\(channelId)"] as? Int{
           ...
           // set value in hash
            ...
            //send delegate
            BumbleConfig.shared.sendAgentUnreadCount(totalUnreadCount)
            BumbleConfig.shared.sendAgentChannelsUnreadCount(channelList.count)
        }else{
...
func pushTotalUnreadCount() {
    ...
    BumbleConfig.shared.sendUnreadCount(chatCounter)
...
func sendUserUnreadCount() {
    ...
    BumbleConfig.shared.notifyUserUnreadCount(json)
}

func getPushUnreadCount() -> Int {
    var count = 0
    for each in BumbleConfig.shared.pushArray {
        count += each.count
    }
    return count
}
...
func updatePushCount(pushInfo: [String: Any]) {
    ...
    let index = BumbleConfig.shared.pushArray.firstIndex { (p) -> Bool in
        return p.channelId == channelId
    }
    var newObj = PushInfo(json: pushInfo)
    guard index != nil else {
        newObj.count = 1
        BumbleConfig.shared.pushArray.append(newObj)
        return
    }
    let oldObj = BumbleConfig.shared.pushArray[index!]
    if let newMuid = newObj.muid, let oldMuid = oldObj.muid, newMuid == oldMuid {
        return
    }
    BumbleConfig.shared.pushArray[index!].count += 1
    BumbleConfig.shared.pushArray[index!].muid = newObj.muid
}
...
func resetForChannel(channelId: Int) {
    ...
    BumbleConfig.shared.pushArray.remove(at: index!)
    
}
func resetPushCount() {
    BumbleConfig.shared.pushArray = []
}
...
func validateCustomerData() -> Bool {
    switch BumbleConfig.shared.credentialType {
    case FuguCredentialType.reseller:
        if BumbleConfig.shared.resellerToken.isEmpty || BumbleConfig.shared.referenceId < 0 {
            return false
        }
    default:
...
func validateAgentData() -> Bool {
    guard BumbleConfig.shared.agentDetail != nil, !BumbleConfig.shared.agentDetail!.fuguToken.isEmpty else {
        return false
    }
    return true
}
...
func currentUserImage() -> String? {
    switch BumbleConfig.shared.appUserType {
    case .agent:
        return BumbleConfig.shared.agentDetail?.userImage
    case .customer:
        return BumbleConfig.shared.userDetail?.userImage?.absoluteString
    }
}
