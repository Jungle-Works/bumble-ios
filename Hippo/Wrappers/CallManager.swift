...
//    }
    ...
    func startGroupCall(call: GroupCallData, groupCallChannelData : GroupCallChannelData, completion: @escaping (Bool, NSError?) -> Void){
        ...
        let callToMake = Call(peer: peer, signalingClient: call.signallingClient, uID: call.muid, currentUser: currentUser, type: getCallTypeWith(localType: call.callType), link: "", isGroupCall: true, jitsiUrl: BumbleConfig.shared.jitsiUrl ?? "")
    ...
    func startCall(call: CallData, completion: @escaping (Bool, NSError?) -> Void) {
        ...
        #if canImport(JitsiMeet)
        let callToMake = Call(peer: peer, signalingClient: call.signallingClient, uID: call.muid, currentUser: currentUser, type: getCallTypeWith(localType: call.callType), link: "", jitsiUrl: BumbleConfig.shared.jitsiUrl ?? "")
        #else
    ...
    func getCurrentUser() -> HippoUser? {
        ...
        case .customer:
            guard let user = BumbleConfig.shared.userDetail else {
                return nil
            }
...
extension CallManager: HippoCallClientDelegate {
    func callStarted(isCallStarted: Bool) {
         BumbleConfig.shared.jitsiOngoingCall = isCallStarted
    }
    
    func loadCallPresenterView(request: CallPresenterRequest) -> CallPresenter? {
        return BumbleConfig.shared.notifyCallRequest(request)
    }
}
#endif
