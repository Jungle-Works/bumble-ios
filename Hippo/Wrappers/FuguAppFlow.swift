...
//        }
//
//        var fuguNewChatAttributes = FuguNewChatAttributes(transactionId: transactionId, userUniqueKey: BumbleConfig.shared.userDetail?.userUniqueKey, otherUniqueKey: nil, tags: HippoProperty.current.newConversationButtonTags, channelName: nil, preMessage: "", groupingTag: nil)
...
//    }
    ...
    func presentPrePaymentController(_ url : String, _ channelId : Int){
        ...
        guard let config = WebViewConfig(url: urlString, title: BumbleStrings.payment) else { return }
        ...
        vc.isPaymentCancelled = {(sucess) in
            BumbleConfig.shared.HippoPrePaymentCancelled?()
        }
        vc.isPaymentSuccess = {(status) in
            BumbleConfig.shared.HippoPrePaymentSuccessful?(status)
        }
    ...
    func toShowInAppNotification(userInfo: [String: Any]) -> Bool {
        ...
        if let muid = userInfo["muid"] as? String {
            if BumbleConfig.shared.muidList.contains(muid) {
                return false
            }
