...
class MessageTableViewCell: UITableViewCell {
    ...
    internal func setSourceIcon() {
        let showMessageSourceIcon = BumbleConfig.shared.appUserType == .agent ? true : HippoProperty.current.showMessageSourceIcon
        let icon =  showMessageSourceIcon ? message?.messageSource.getIcon() :  nil
        sourceIcon?.image = icon
        sourceIcon?.isHidden = icon == nil
        sourceIcon?.tintColor = BumbleConfig.shared.theme.sourceIconColor
    }
