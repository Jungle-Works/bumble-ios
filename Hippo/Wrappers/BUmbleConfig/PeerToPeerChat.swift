    ...
    */
    public init?(uniqueChatId: String?, myUniqueId: String, idsOfPeers: [String], channelName: String, peerName: String = "", otherUserImage: String? = nil) {
        ...
        guard parsedPeers.count > 0 else {
            BumbleConfig.shared.log.error("id of peers are nil", level: .error)
            return nil
        }
    ...
    func generateParamForUnreadCount() throws -> [String: Any] {
        ...
        guard BumbleConfig.shared.appUserType == .customer else {
            BumbleConfig.shared.log.trace("notAllowedForAgent", level: .error)
            throw HippoError.notAllowedForAgent
        }
        
        guard !appSecretKey.isEmpty else {
            BumbleConfig.shared.log.trace("appSecret key found empty", level: .error)
        ...
        guard !enUserId.isEmpty else {
            BumbleConfig.shared.log.trace("updateUserDetail", level: .error)
            throw HippoError.updateUserDetail
        }
        json["en_user_id"] = enUserId
        
        json["user_unique_key"] = BumbleConfig.shared.userDetail?.userUniqueKey ?? userUniqueId
