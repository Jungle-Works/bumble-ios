...
static let devUrl = "https://test-api-3040.tookanapp.com/"//"https://hippo-api-dev.fuguchat.com:3002/"//
...
@objcMembers public class BumbleConfig : NSObject {
    
    public static var shared = BumbleConfig()
...
//     }
    ...
    func getDeepLinkData(_ data : [String : Any]){
        BumbleConfig.shared.delegate?.deepLinkClicked(response: data)
    }
...
//    }
    ...
    internal func setAgentStoredData_bumble() {
        ...
        BumbleConfig.shared.appUserType = .agent
        BumbleConfig.shared.agentDetail = AgentDetail(dict: storedData)
        
        if !BumbleConfig.shared.agentDetail!.fuguToken.isEmpty {
            BumbleConfig.shared.appUserType = .agent
        }
    }
...
//            print("customerLogout")
    ...
    public static func setBotGroupID(id: Int) {
        guard  id >= 0  else {
            BumbleConfig.shared.log.error("Bot Id Should be greater then zero!!!!", level: .error)
            return
        }
        ...
        if userInfo["is_announcement_push"] as? Bool == true, let channel_id = userInfo["channel_id"] as? Int{
            if !(getLastVisibleController() is PromotionsViewController){
                if var channelArr = UserDefaults.standard.value(forKey: DefaultName.announcementUnreadCount.rawValue) as? [String]{
                    ...
                    BumbleConfig.shared.announcementUnreadCount?(channelArr.count)
                }
            }
        }else{
            if let data = P2PUnreadData.shared.getData(with: userInfo["chat_transaction_id"] as? String ?? ""), let otherUserUniqueKey = ((userInfo["user_unique_key"] as? [String])?.filter{$0 != BumbleConfig.shared.userDetail?.userUniqueKey}.first){
...
//        }
    ...
    //                HippoChat.isSingleChatApp = false
                    BumbleConfig.shared.presentPromotionalPushController()
                    return
                }
            }
        }
...
extension BumbleConfig {
    func sendp2pUnreadCount(_ unreadCount : Int, _ channelId : Int){
        BumbleConfig.shared.delegate?.sendp2pUnreadCount(unreadCount: unreadCount,channelId: channelId)
    }
    
    func sendDataIfChatIsAssignedToSelfAgent(_ dic : [String : Any]){
        BumbleConfig.shared.delegate?.sendDataIfChatIsAssignedToSelfAgent(dic)
    }
    
    func sendAgentUnreadCount(_ totalCount: Int) {
       // showAlertWith(message: "\(totalCount)", action: nil)
        BumbleConfig.shared.delegate?.hippoAgentTotalUnreadCount(totalCount)
        print("sendAgentUnreadCount====================",totalCount)
    }
    
    func sendAgentChannelsUnreadCount(_ totalCount: Int) {        BumbleConfig.shared.delegate?.hippoAgentTotalChannelsUnreadCount(totalCount)
        print("sendAgentChannelsUnreadCount====================",totalCount)
    }
    
    func sendUnreadCount(_ totalCount: Int) {
        BumbleConfig.shared.unreadCount?(totalCount)
        BumbleConfig.shared.delegate?.hippoUnreadCount(totalCount)
    }
    
    func notifyUserUnreadCount(_ usersCount: [String: Int]) {
        BumbleConfig.shared.usersUnreadCount?(usersCount)
        BumbleConfig.shared.delegate?.hippoUserUnreadCount(usersCount)
    }
    func notifiyDeinit() {
        BumbleConfig.shared.HippoDismissed?(true)
        BumbleConfig.shared.delegate?.hippoDeinit()
    }
    func notifyDidLoad() {
        BumbleConfig.shared.HippoDismissed?(false)
        BumbleConfig.shared.delegate?.hippoDidLoad()
    }
    ...
    func notifyCallRequest(_ request: CallPresenterRequest) -> CallPresenter? {
        if BumbleConfig.shared.delegate == nil {
           BumbleConfig.shared.log.error("To Enable video/Audio CALL, SET  HippoConfig.shared.setHippoDelegate(delegate: HippoDelegate)", level: .error)
        }
        return BumbleConfig.shared.delegate?.loadCallPresenterView(request: request)
    }
    #endif
}
extension BumbleConfig{
