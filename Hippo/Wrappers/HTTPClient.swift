...
class HTTPClient {
    ...
    func makeSingletonConnectionWith(method: HttpMethodType, identifier: String, showAlert: Bool = true, showAlertInDefaultCase: Bool = true, showActivityIndicator: Bool = true, para: [String: Any]? = nil, baseUrl: String = BumbleConfig.shared.baseUrl, extendedUrl: String, callback: @escaping ServiceResponse) {
...
//            "device_details": AgentDetail.getDeviceDetails(),
    ...
    @discardableResult
    class func makeMultiPartRequestWith(method: HttpMethodType, showAlert: Bool = false, showAlertInDefaultCase: Bool = false, showActivityIndicator: Bool = false, para: [String: Any]? = nil, baseUrl: String = BumbleConfig.shared.baseUrl, extendedUrl: String, fileList: [HippoFile], callback: @escaping ServiceResponse) -> URLSessionDataTask? {
        ...
        let dataTask = URLSession.shared.dataTask(with: request) {
            ...
            DispatchQueue.main.async {
                ...
                do {
                    ...
                    BumbleConfig.shared.log.error("API RESPONSE: ---url: \(urlResponse?.url?.absoluteString ?? "NO URL"), ---data: \(data?.count ?? -1) ---Error: \(error?.localizedDescription ?? "no error")", level: .custom)
