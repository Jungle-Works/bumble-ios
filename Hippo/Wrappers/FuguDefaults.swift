...
class FuguDefaults: NSObject {
    class func set(value: Any?, forKey keyName: String) {
        ...
        } catch { BumbleConfig.shared.log.trace(error, level: .customError) }
    }
    ...
    class func removeObject(forKey keyName: String) {
        ...
        if fileManager.fileExists(atPath: fileUrl.path) {
            ...
            } catch let error as NSError { BumbleConfig.shared.log.trace(error, level: .customError) }
        }
    }
    ...
    class func fuguContentDirectoryURL() -> URL? {
        ...
        if FileManager.default.fileExists(atPath: documentsDirectoryUrl.path, isDirectory: &isDir) == false {
            do {
                ...
                BumbleConfig.shared.log.trace(error, level: .customError)
                return nil
            }
        }
    ...
    class func totalImagesInImagesFlder() -> Int {
        ...
        do {
            ...
            BumbleConfig.shared.log.trace(directoryContents, level: .custom)
            return directoryContents.count
        } catch {
            BumbleConfig.shared.log.trace(error.localizedDescription, level: .customError)
            return -1
        }
    }
    ...
    class func deleteFile(atFilePath filePath: String) {
        ...
        if fileManager.fileExists(atPath: filePath) {
            ...
            catch let error as NSError { BumbleConfig.shared.log.trace(error.localizedDescription, level: .customError) }
        }
    }
}
