...
//        }
   ...
   override  func viewWillDisappear(_ animated: Bool) {
    ...
    func navigationSetUp() {
        ...
        if BumbleConfig.shared.theme.sendBtnIcon != nil {
            
            if let tintColor = BumbleConfig.shared.theme.sendBtnIconTintColor {
                sendMessageButton.imageView?.tintColor = tintColor
            }else{
                sendMessageButton.imageView?.tintColor = BumbleConfig.shared.theme.themeColor
            }
        ...
        if BumbleConfig.shared.theme.addButtonIcon != nil {
            ...
            if let tintColor = BumbleConfig.shared.theme.addBtnTintColor {
                addFileButtonAction.imageView?.tintColor = tintColor
            }else{
                addFileButtonAction.imageView?.tintColor = BumbleConfig.shared.theme.themeColor
            }
            addFileButtonAction.setImage(BumbleConfig.shared.theme.addButtonIcon, for: .normal)
...
//      }
    ...
    func setUpSuggestionsDataAndUI(){
        if BumbleConfig.shared.isSuggestionNeeded == false{
        ...
        if self.messagesGroupedByDate.count > 0 {
            ...
            if givenMessagesArray.count >= BumbleConfig.shared.maxSuggestionCount {
                suggestionContainerView.isHidden = true
                return
            }
        }
    ...
    private func checkAutoSuggestions() {
        ...
        //}else if let lastMessage = getLastMessage(), lastMessage.type == MessageType.normal{
           ...
           //try {
            if let id = BumbleConfig.shared.questions[lastMessage.message]{
                if id > -1 {
                    suggestionContainerView.isHidden = false
                    updateData(id: id)
                    return
...
//        }
    ...
    private func updateData(id: Int) {
        ...
        if let suggestionsIdsArr = BumbleConfig.shared.mapping[id]{
            ids = suggestionsIdsArr
            if ids.count > 0{
                for i in 0..<ids.count{
                    if let suggestionsStr = BumbleConfig.shared.suggestions[ids[i]] {
...
//            startGettingNewMessages()
        ...
        MessageStore.getMessages(requestParam: request, ignoreIfInProgress: false) {[weak self] (response, isCreateConversationRequired)  in
            ...
            guard let result = response, result.isSuccessFull, let weakself = self else {
                completion?(false)
                if BumbleConfig.shared.shouldShowSlowInternetBar ?? true{
                  self?.goForApiRetry()
                }
                return
            }
    ...
    func handleVideoIcon() {
        ...
        if isDirectCallingEnabledFor(type: .video) {
            ...
            view_Navigation.video_button.tintColor = BumbleConfig.shared.theme.headerTextColor
            view_Navigation.video_button.isEnabled = true
            view_Navigation.video_button.setImage(BumbleConfig.shared.theme.videoCallIcon, for: .normal)
    ...
    func handleAudioIcon() {
        ...
        if isDirectCallingEnabledFor(type: .audio) {
            view_Navigation.call_button.tintColor = BumbleConfig.shared.theme.headerTextColor
            view_Navigation.call_button.isEnabled = true
            view_Navigation.call_button.setImage(BumbleConfig.shared.theme.audioCallIcon, for: .normal)
...
//   }
    ...
    func handleRequestForCreateConersationForGetMessages(error: MessageStore.GetMessagesError?, completion: ((_ success: Bool) -> Void)?) {
        guard let result = error, result.isCreateConversationRequired, BumbleConfig.shared.userDetail?.userUniqueKey != nil else {
            completion?(false)
            return
        }
...
extension ConversationsViewController {
    ...
    func configureChatScreen() {
        ...
        if BumbleConfig.shared.theme.chatbackgroundImage != nil      {
            tableViewChat.backgroundColor = .white//.clear
            backgroundImageView.image = BumbleConfig.shared.theme.chatbackgroundImage
            backgroundImageView.contentMode = .scaleToFill
        }
//        self.messageTextView.textAlignment = .left
        self.messageTextView.font = BumbleConfig.shared.theme.typingTextFont
        self.messageTextView.textColor = BumbleConfig.shared.theme.typingTextColor
    ...
    func expectedHeight(OfMessageObject chatMessageObject: HippoMessage) -> CGFloat {
       ...
       height: CGFloat.greatestFiniteMagnitude)
        ...
        if isOutgoingMsg == true {
            ...
            attributes = [NSAttributedString.Key.font: BumbleConfig.shared.theme.inOutChatTextFont]
...
//        let marginBetweenHeaderAndDescription = CGFloat(2.5)
        ...
        let headerFont = BumbleConfig.shared.theme.actionableMessageHeaderTextFont
        let descriptionFont = BumbleConfig.shared.theme.actionableMessageDescriptionFont
        let priceFont = BumbleConfig.shared.theme.actionableMessagePriceBoldFont
...
//    }
    ...
    func newMessageReceived(newMessage message: HippoMessage) {
        guard !isSentByMe(senderId: message.senderId) || message.type.isBotMessage  else {
            BumbleConfig.shared.log.debug("Yahaa se nahi nikla", level: .custom)
            return
        }
