...
//    }
    ...
    func navigationSetUp() {
         ...
         if BumbleConfig.shared.theme.sendBtnIcon != nil {
             sendMessageButton.tintColor = BumbleConfig.shared.theme.themeColor
        ...
        if BumbleConfig.shared.theme.moreOptionsButtonIcon != nil {
                moreOptionsButton.tintColor = BumbleConfig.shared.theme.themeColor
                moreOptionsButton.setImage(BumbleConfig.shared.theme.moreOptionsButtonIcon, for: .normal)
...
//        manageSendButton()
    ...
    func assignChatToSelf() {
        AgentConversationManager.assignChatToMe(channelID: self.channelId) {[weak self] (success) in
            ...
            strongSelf.channel?.chatDetail?.assignedAgentName = BumbleConfig.shared.agentDetail?.fullName ?? ""
    ...
    override func getMessagesBasedOnChannel(fromMessage pageStart: Int, pageEnd: Int?, completion: ((_ success: Bool) -> Void)?) {
        ...
        if BumbleConfig.shared.agentDetail?.fuguToken == nil {
...
extension AgentConversationViewController {
    ...
    func handleInfoIcon() {
        ...
        view_Navigation.info_button.setImage(BumbleConfig.shared.theme.informationIcon, for: .normal)
        view_Navigation.info_button.addTarget(self, action:  #selector(infoButtonClicked), for: UIControl.Event.touchUpInside)
        view_Navigation.info_button.tintColor = BumbleConfig.shared.theme.headerTextColor
...
//        botButton.isHidden = isPrivate || config.shouldHideBottonButtons()
        ...
        if isBotButtonHidden == true{
            ...
            self.attachments.append(Attachment(icon : BumbleConfig.shared.theme.botIcon  , title : "Bot"))
        }
