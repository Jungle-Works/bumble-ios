...
//    }
    ...
    func checkForAnyError() {
        ...
        if  BumbleConfig.shared.agentDetail == nil || BumbleConfig.shared.agentDetail!.oAuthToken.isEmpty {
...
//        broadCastButton.isHidden = !HippoConfig.shared.isBroadcastEnabled
        ...
        if BumbleConfig.shared.theme.filterBarButtonText.count > 0 {
            view_NavigationBar.rightButton.setTitle((" " + BumbleConfig.shared.theme.filterBarButtonText), for: .normal)
            if BumbleConfig.shared.theme.filterBarButtonFont != nil {
                view_NavigationBar.rightButton.titleLabel?.font = UIFont.regular(ofSize: 14)//HippoConfig.shared.theme.filterBarButtonFont
            }
            view_NavigationBar.rightButton.setTitleColor(BumbleConfig.shared.theme.filterBarButtonTextColor, for: .normal)
        } else {
            setFilterButtonIcon()
        }
...
//        refreshControl.tintColor = .themeColor
        if BumbleConfig.shared.theme.themeColor == UIColor.white || BumbleConfig.shared.theme.themeColor == UIColor.clear{
            refreshControl.tintColor = BumbleConfig.shared.theme.themeTextcolor
        }else{
            refreshControl.tintColor = .themeColor
        }
