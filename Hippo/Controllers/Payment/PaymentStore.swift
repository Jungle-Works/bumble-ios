...
//            return
//        }
        guard let accessToken = BumbleConfig.shared.agentDetail?.fuguToken, let channelId = self.channelId else {
            completion(false, nil)
            return
        }
        ...
        HTTPClient.makeConcurrentConnectionWith(method: .POST, para: param, extendedUrl: AgentEndPoints.sendPayment.rawValue) { (response, error, _, statusCode) in
            ...
            guard let responseDict = response as? [String: Any],
                let statusCode = responseDict["statusCode"] as? Int, statusCode == 200 else {
                    BumbleConfig.shared.log.debug("API_SendPayment ERROR.....\(error?.localizedDescription ?? "")", level: .error)
                    completion(false, error)
                    return
            }
            completion(true, nil)
...
//        Helper.log.debug("Parama == \(param)", level: .request)
        BumbleConfig.shared.log.debug("Parama == \(param)", level: .request)
