...
extension BroadcastSelectionViewController {
    ...
    internal func setUpView() {
        ...
        if BumbleConfig.shared.theme.leftBarButtonText.count > 0 {
            backButton.setTitle((" " + BumbleConfig.shared.theme.leftBarButtonText), for: .normal)
            if BumbleConfig.shared.theme.leftBarButtonFont != nil {
                backButton.titleLabel?.font = BumbleConfig.shared.theme.leftBarButtonFont
            }
        ...
        case .selectTeam:
            ...
            self.navigationItem.title = BumbleConfig.shared.strings.selectTeamsString
        case .selectAgent:
        ...
        default:
            self.navigationItem.title = BumbleConfig.shared.theme.broadcastHeader
        }
        handleApplyButtonView()
    }
    
    func handleApplyButtonView() {
        applyButton.backgroundColor = BumbleConfig.shared.theme.headerBackgroundColor
        applyButton.setTitleColor(BumbleConfig.shared.theme.headerTextColor, for: .normal)
    ...
    func startLoading() {
        loadingLabel.text = "Loading \(BumbleConfig.shared.strings.displayNameForCustomers) for \(broadcaster?.selectedTeam?.tagName ?? "")"
...
extension BroadcastSelectionViewController: UITableViewDataSource {
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        case .selectAgent:
            ...
            labelBgView.backgroundColor = BumbleConfig.shared.theme.backgroundColor
            
            let label = UILabel()
            label.text = "Team: " + (broadcaster?.selectedTeam?.tagName ?? "")
            label.textColor = BumbleConfig.shared.theme.headerBackgroundColor
