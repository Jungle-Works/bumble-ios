...
//        let theme = HippoTheme.theme
        totalPriceLabel.textColor = BumbleConfig.shared.theme.broadcastTitleColor
        totalPriceLabel.font = UIFont.regular(ofSize: 15)
        showMoreButton.setTitleColor(BumbleConfig.shared.theme.broadcastTitleColor, for: .normal)
        showMoreButton.titleLabel?.font = UIFont.regular(ofSize: 15)
        showMoreButton.tintColor = BumbleConfig.shared.theme.themeTextcolor
    }
    ...
    func setTotalCostLabel() {
        ...
        if price > 0 {
            ...
            totalPriceLabel.text = "\(BumbleStrings.totalPrice): \(formattedValue)"
        } else {
