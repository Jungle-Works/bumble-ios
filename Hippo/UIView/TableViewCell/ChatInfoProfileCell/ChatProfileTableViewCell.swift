...
class ChatProfileTableViewCell: UITableViewCell {
    ...
    func setupCell(with data: ChatDetail) {
        ...
        view_Email.isHidden = (BussinessProperty.current.shouldHideCustomerData ?? false) && BumbleConfig.shared.agentDetail?.agentUserType == .agent  ? true : false
        view_Phone.isHidden = (BussinessProperty.current.shouldHideCustomerData ?? false)  && BumbleConfig.shared.agentDetail?.agentUserType == .agent ? true : false
    }
